...
if(file_exists($ban))
{
    $info = file_get_contents($ban);
    $ban_album_names = explode(';',$info);
}
...
function word_array()
{
    $sql = 'SELECT * FROM `eve_online_pilotki` k';
    $fetch = fetch_al($sql);
    return $fetch;
}

function fetch_al($sql,$ret=array()) {
	global $db;
	try {
		if( $result = $db->query($sql) ) {
...
function build_table($data,$title=null,$ancor=null,$add_rank=true,$ext_comment=null,$print_output=true) {
		...
		foreach( $data as $row1 ) {
			...
			#		}
				...
				if( in_array($title,array('Corp_dolg')) ) {
					$align = 'right';
					$htmlspecialchars = false;
					$data = '<span style="color:red;">'.isknf($data, ($title=='Corp_dolg')).'</span>';
			#		if( @$_GET['pswc'] == 'ee_hide_bountys' ) {
			#			$data = '<i>hidden</i>';
			#		}
				}
	...
	//////////////////////////"""""""""""""""""""""""""""""########################
						...
						$tmp = $row1;
						if( isset($row1['_kb_kills_30days']) ) {
							$tmp['kb_kills_30days'] = $row1['_kb_kills_30days'];
						}			
					#if( isset($tmp['kb_kills_30days']) ):
...
#						}
                        ...
                        if(!$_GET['from'])
						$data = sprintf('<a %s %s href="%s&period=%s&char=%s">%s</a>',$overlib,( $css ? 'style="'.$css.'"' : '' ),$file,$_GET['period'],urlencode($data),$data);
                        else
						$data = sprintf('<a %s %s href="%s&char=%s&from=%s&tomuch=%s">%s</a>',$overlib,( $css ? 'style="'.$css.'"' : '' ),$file,urlencode($data),$_GET['from'],$_GET['tomuch'],$data);
					}
