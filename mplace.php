...
if($c = $simpla->request->get('code'))
{
	...
	if($new_password = $simpla->request->post('new_password'))
	{
		...
		$htaccess_file = $simpla->config->root_dir.'fetch/.htaccess';
		$passwd_file = $simpla->config->root_dir.'fetch/.passwd';
		...
		else
		{
			...
			$htaccess = preg_replace('/AuthUserFile[\s]+[^\r\n]+/', 'AuthUserFile '.$simpla->config->root_dir.'fetch/.passwd', $htaccess);
			...
			print "
				...
				<p>
				<a href='".$simpla->root_url."/fetch'>Перейти в панель управления</a>
				</p>
			";
		}
	}
