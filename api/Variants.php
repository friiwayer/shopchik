...
class Variants extends Simpla
{
	...
	**/
	...
	{		
		...
		$query = $this->db->placehold("SELECT v.id, v.product_id , v.ems, v.deadline, v.dprice, v.d_day, v.price, NULLIF(v.compare_price, 0) as compare_price, v.sku, IFNULL(v.stock, ?) as stock, (v.stock IS NULL) as infinity, v.name, v.attachment, v.position
	...
	{	
		...
		$query = $this->db->placehold("SELECT v.id, v.product_id , v.price, NULLIF(v.compare_price, 0) as compare_price, v.ems, v.dprice, v.deadline, v.d_day, v.sku, IFNULL(v.stock, ?) as stock, (v.stock IS NULL) as infinity, v.name, v.attachment
	...
	public function get_variant_f()
	{			
		$query = $this->db->placehold("SELECT v.id, v.product_id , v.price, NULLIF(v.compare_price, 0) as compare_price, v.dprice,  v.ems, v.deadline, v.d_day, v.sku, IFNULL(v.stock, ?) as stock, (v.stock IS NULL) as infinity, v.name, v.attachment
					FROM __variants v
					LIMIT 1", $this->settings->max_order_amount);
    ...
    {
        $query = $this->db->placehold("UPDATE __variants SET `deadline` = 0,`price` = `dprice` WHERE `d_day` <= NOW()  AND `deadline` = 1 LIMIT 1");
        $this->db->query($query);
    }
