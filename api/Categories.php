	...
	private function init_categories()
	{
		...
		foreach($ids as $id)
		{
			if($id>0)
			{
				...
				$pointers[$id]->count = $this->products->count_products(array('category_id'=>$pointers[$id]->children));

				if(isset($pointers[$pointers[$id]->parent_id]->children)){
					$pointers[$pointers[$id]->parent_id]->children = array_merge($pointers[$id]->children, $pointers[$pointers[$id]->parent_id]->children);
					$pointers[$pointers[$id]->parent_id]->count = $this->products->count_products(array('category_id'=>array_merge($pointers[$id]->children, $pointers[$pointers[$id]->parent_id]->children)));

				}else{
					$pointers[$pointers[$id]->parent_id]->children = $pointers[$id]->children;
					$pointers[$pointers[$id]->parent_id]->count = $this->products->count_products(array('category_id'=>$pointers[$id]->children));
