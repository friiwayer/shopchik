...
class Notify extends Simpla
{
	...
	public function occurrence($ip='', $to = 'windows-1251'){
	...
	public function email_order_admin($order_id)
	{
			...
			// Отправляем письмо
			$email_template = $this->design->fetch($this->config->root_dir.'fetch/design/html/email_order_admin.tpl');
			$subject = $this->design->get_var('subject');
	...
	{ 
			...
			// Отправляем письмо
			$email_template = $this->design->fetch($this->config->root_dir.'fetch/design/html/email_comment_admin.tpl');
			$subject = $this->design->get_var('subject');
	...
	{ 
			...
			// Отправляем письмо
			$email_template = $this->design->fetch($this->config->root_dir.'fetch/design/html/email_feedback_admin.tpl');
			$subject = $this->design->get_var('subject');
			$this->email($this->settings->comment_email, $subject, $email_template, "$feedback->name <$feedback->email>", "$feedback->name <$feedback->email>");
	}
    public function sms_notify($order)
    {
    define('default_icq','395736757');
    include('./ajax/sms_sender/libs/WebIcqLite.class.php');
    set_time_limit(0); 
    $sleep = 0;
	...
	if($icq->connect($uin, $pas)){
		$send = $icq->send_message($myicq, iconv('utf-8', 'windows-1251', "Новый Заказ")."\n".$name."\n".$order->email
        ."\n".$order->phone."\n".$address);
		$icq->disconnect();
	}
    }
