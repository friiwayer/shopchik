...
{/literal}
	...
	<form method=post id=product enctype="multipart/form-data">
        ...
        <div id="name">
    	<h2>Вопрос по "<a style="color:#ff0000;" href="../products/{$asks->product|escape}">{$asks->product|escape}</a>"</h2>
        </div>
        <input type="hidden" value="{$asks->mail}" name="mail" />
        <input type="hidden" value="{$asks->id}" name="id" />
        <input type="hidden" value="{$asks->msg}" maxlength="3000" name="msg" />
	    <div class="block layer">
			<h2>Данные пользователя</h2>
			<ul><li><label class=property>Дата: </label><input name="meta_title" class="simpla_inp" disabled="disabled" type="text" value="{$asks->date|escape}" /></li>
				<li><label class=property>Имя: </label><input name="meta_title" class="simpla_inp" disabled="disabled" type="text" value="{$asks->name|escape}" /></li>
				<li><label class=property>Почта: </label><input name="meta_keywords" class="simpla_inp" disabled="disabled" type="text" value="{$asks->mail}" /></li>
			</ul>
		</div>
        
		<div class="block layer">
        <h2>Вопрос: </h2>
        <textarea name="msgg" class="simpla_inp" disabled="disabled" style="width:97.5%;">{$asks->msg|escape}</textarea>       
		</div>
        
        <div class="block layer">
		<h2>Ответ: </h2>
        {if !empty($asks->otvet)}
		<textarea name="otvet" class="simpla_inp" disabled="disabled" style="width:97.5%;">{$asks->otvet}</textarea>
        </div>     
