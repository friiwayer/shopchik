...
<input type=hidden name="session_id" value="{$smarty.session.id}"/>
 	...
 	<div id="name">
		...
		<div class="checkbox">
			<input name=showm value='1' type="checkbox" id="show_m_checkbox" class="{$product->show_m}" {if $product->show_m}checked{/if}/> <label for="show_m_checkbox">На главной</label>
		</div>        
	</div> 
	...
	<div id="variants_block" {assign var=first_variant value=$product_variants|@first}{if $product_variants|@count <= 1 && !$first_variant->name}class=single_variant{/if}>
		...
		<ul>
            ...
            <li class="variant_ems">       <input name="variants[ems][{$variant->id}]"           type="text"   value="{$variant->ems|escape}" /></li>
            ...
            <li class="variant_deadline">  <input name="variants[deadline][{$variant->id}]"      value="{$variant->deadline}"  class="oni"     type="text" size="1" {if $variant->deadline}checked{/if} /></li>
            <li class="variant_d_day"> <input name="variants[d_day][{$variant->id}]"         type="text"   value="{$variant->d_day|escape}"  class="dline" size="10"  /></li>
            <li class="variant_dprice"> <input name="variants[dprice][{$variant->id}]"         type="text"   value="{$variant->dprice|escape}"  class="" size="10"  /></li>
	...
	<div id="column_left">
		...
		<div class="block layer">
			...
			<ul>
                ...
                <li><label class=property>Код на видео с YouTube</label><input name="youtube" class="simpla_inp" type="text" value="{$product->youtube|escape}" /></li>
                <li><label class=property>Ссылка на поставщика(заказ)</label><textarea name="postav" class="simpla_inp" />{$product->postav|escape}</textarea></li>
                ...
                <li><label class=property>Ссылка на товар: </label><a href="{$product->postav}" target="_blank">{$product->name|escape}</a></li>
			</ul>
		</div>
