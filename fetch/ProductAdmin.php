...
class ProductAdmin extends Simpla
{
	public function fetch()
	{
		...
		if($this->request->method('post') && !empty($_POST))
		{
            ...
            $product->show_m = $this->request->post('showm', 'boolean');
            
            
			$product->url = $this->request->post('url', 'string');
            $product->youtube = $this->request->post('youtube');
            $product->postav = $this->request->post('postav');
            ...
            if($this->request->post('type_ac','string') && $this->request->post('prod_id','integer') && $this->request->post('price_ac') != '')
            {
                $this->products->add_accesuar($this->request->post('prod_id'), $this->request->post('type_ac'), $this->request->post('price_ac'));
            }
            ...
            $apparat = $this->request->post('apparat');
            if(is_array($apparat))
            {
                foreach ($apparat as $v=>$k)
                {
                    $rt[$v]->feature_id = $v;
                    $rt[$v]->value = $k;
                }
  	    		...
  	    		else
  	    		{
   	    		...
   	    		if($product->id)
   	    		{
					...
					foreach($options as $option)
					{
	  	    		...
	  	    		if(is_array($apparat))
					foreach($apparat as $app)
					{
						if(in_array($app->feature_id, $category_features))
							$this->features->update_option($product->id, $app->feature_id, $app->value);
					}					
		...
		else
		{
			...
			if($product && $product->id)
			{
				...
				$product_yt = $this->products->youtube_tovar($product->id);
				// Варианты товара
                //$product->show_m = $this->products->get_products(array('product_id'=>$product->id, 'show_m'));
				...
				$product->show_m = $this->products->show_inmain($product->id);
				// Связанные товары
				$related_products = $this->products->get_related_products(array('product_id'=>$product->id));
                $axes = $this->products->get_ax_products(array('product_id'=>$product->id));
			}
			else
			{
				// Сразу активен
            ...
            $show_accs = $this->products->select_accall();
            $show_acc = $this->products->select_accesuars($product->id);
			
            foreach($show_acc as $show_c)
            {
            if($_POST['deletz'] == $show_c->id)
			{
			 $this->products->delete_accesuars($show_c->id);
	        }                
            }
            
            $this->design->assign('show_acc', $show_acc);*/ 
        ...
        $display = $this->features->get_features_display();
        $hard = $this->features->get_features_hard();
        $multi = $this->features->get_features_multi();
        $compl = $this->features->get_features_compl();

        $erc = $this->blog->col_sabjs();
        $this->design->assign('erc', $erc);
        
        $this->design->assign('compl', $compl);
        $this->design->assign('multi', $multi);
        $this->design->assign('hard' ,$hard);
        $this->design->assign('display', $display);
