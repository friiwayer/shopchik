...
class View extends Simpla
{
	...
	public function __construct()
	{
		...
		else
		{
            ...
            $this->design->smarty->registerPlugin("function", "get_staty",               array($this, 'get_staty_plugin'));                                                                                                         
            $this->design->smarty->registerPlugin("function", "get_last_statya",         array($this, 'get_last_statya_plugin'));
            $this->design->smarty->registerPlugin("function", "get_tags",                array($this, 'get_tags_plugin'));
			$this->design->smarty->registerPlugin("function", "get_brands",              array($this, 'get_brands_plugin'));
            $this->design->smarty->registerPlugin("function", "get_ratings",             array($this, 'get_ratings_plugin'));
            ...
            $this->design->smarty->registerPlugin("function", "get_last_news",           array($this, 'get_last_news_plugin'));
            $this->design->smarty->registerPlugin("function", "get_date",             array($this, 'get_date_plugin'));
            $this->design->smarty->registerPlugin("function", "get_random",             array($this, 'get_random_products_plugin'));
            $this->design->smarty->registerPlugin("function", "get_dl",             array($this, 'get_deadline_products_plugin'));
		}
	}
    ...
    public function get_date_plugin()
    {
        $date->year = date('Y');
        return $date->year;
    }
    ...
    public function get_corp_plugin($params, &$smarty)
    {$smarty->assign($params['var'], $this->products->get_count_cat_p('22'));
    }        
    ...
    public function get_last_news_plugin($params, &$smarty)
    {       
			$smarty->assign($params['var'], $this->blog->get_post('max'));        
    }

    public function get_last_statya_plugin($params, &$smarty)
    {       $id_blog = $this->blog->get_statya('max');
    
            if(is_object($id_blog) && !empty($id_blog->id))
            $images = $this->blog->get_images(array('blog_id'=>$id_blog->id));
            {foreach($images as $image)
            {
            $id_blog->image = $image;}
            }                       
			$smarty->assign($params['var'], $id_blog);        
    }
     	
    public function get_staty_plugin($params, &$smarty)
    {
			$smarty->assign($params['var'], $this->blog->get_staty($params));        
    }
    ...
    public function get_tags_plugin($params, &$smarty)
    {   $mass = $this->products->tags_clowd();
        if(!isset($params['visible']))
		$params['visible'] = 1;
        if(!empty($params['var']) && is_array($mass) && count($mass)==$params['limit'])
        $smarty->assign($params['var'], $mass);
    }
    
    public function get_ratings_plugin($params, &$smarty)
    {
		if(!isset($params['visible']))
			$params['visible'] = 1;
		$params['featured'] = 1;
		if(!empty($params['var']))
		{
			foreach($this->products->get_most_rated($params) as $p)
				$products[$p->id] = $p;
			if(!empty($products))
			{
				// id выбраных товаров
				...
				foreach($variants as &$variant)
				{
                    ...
                    $products[$variant->product_id]->ccoms = $this->comments->count_comments(array('object_id'=>$variant->product_id));
				}
	...
	public function get_deadline_products_plugin ($params, &$smarty)
    {
		$params['deadline'] = 1;
		if(!empty($params['var']))
		{
			...
			if(!empty($products))
			{
                ...
                $feates = $this->features->get_product_slider(array('product_id'=>$products_ids));
                
                foreach($feates as $feature)
                {
                    $products[$feature->product_id]->feates[] = $feature;
	...
	public function get_discounted_products_plugin($params, &$smarty)
	{
    	...
    	public function get_random_products_plugin($params, &$smarty)
	{
		if(!isset($params['visible']))
			$params['visible'] = 1;
		$params['random'] = 1;
