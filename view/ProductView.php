...
class ProductView extends View
{
	...
	{   
        ...
        $product->youtube = &$this->products->youtube_tovar($product->id);
        ...
        $product->main = $this->features->get_product_optionz(array('product_id'=>$product->id));
	    $product->hards = $this->features->get_product_hard(array('product_id'=>$product->id));
        $product->display = $this->features->get_product_displ(array('product_id'=>$product->id));
        $product->multimedia = $this->features->get_product_mult(array('product_id'=>$product->id));
        $product->compl = $this->features->get_product_compl(array('product_id'=>$product->id));
        $product->net = $this->features->get_product_net(array('product_id'=>$product->id));
        
		// Автозаполнение имени для формы комментария
		if(!empty($this->user))
			$this->design->assign('comment_name', $this->user->name);

        if($this->request->post('uname','string') && $this->request->post('sabj'))
        {   
            $mail = $this->request->post('mail');
            $name = $this->request->post('uname');
            $text = $this->request->post('sabj');
            $tovname = $this->request->post('productz');
            $tov_id = $this->request->post('tov_id');
            if(!empty($mail) && !empty($name) && !empty($text))
            {$this->products->insert_ask($mail, $text, $tovname, $name, $tov_id);}
        }
        ...
        $show_acx = $this->products->select_accesuars($product->id);
        $this->design->assign('show_acx',$show_acx);
